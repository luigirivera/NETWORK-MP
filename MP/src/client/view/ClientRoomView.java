...
public class ClientRoomView extends JFrame implements ClientObserver{
	...
	public ClientRoomView(ClientModel model, ClientController controller, String destination, String name) {
	...
	private void init() {
		...
		leave = new JButton("Leave Chatroom");
		...
		public void keyPressed(KeyEvent e) {
			...
			if (e.getKeyCode()==KeyEvent.VK_ENTER) {
				if (!message.getText().trim().isEmpty()) {
					controller.sendRoom(destination, message.getText());
					message.setText("");
				}
			}
		}
	...
	class SendFileListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(file!=null)
			{
				...
				switch(result) {
				case JOptionPane.OK_OPTION:
					controller.sendRoom(destination, file);
					if (!message.getText().trim().isEmpty()) {
						controller.sendRoom(destination, message.getText().trim());
					}
					break;
				default:
					break;
	...
	public void update() {
		...
		if(last.getScope()==MessageScope.ROOM && last.getDestination().equalsIgnoreCase(this.destination)) {
